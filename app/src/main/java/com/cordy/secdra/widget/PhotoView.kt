...
class PhotoView : ImageView {
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        if (p.width == ViewGroup.LayoutParams.MATCH_PARENT) {
            ...
            if (widthMode == View.MeasureSpec.EXACTLY) {
                ...
                width = if (drawableW > pWidth) pWidth else drawableW
            } else {
                width = drawableW
            }
        }
        ...
        if (p.height == ViewGroup.LayoutParams.MATCH_PARENT) {
            ...
            if (heightMode == View.MeasureSpec.EXACTLY) {
                ...
                height = if (drawableH > pHeight) pHeight else drawableH
            } else {
                height = drawableH
            }
        }
    ...
    fun canScrollHorizontallySelf(direction: Float): Boolean {
        ...
        return if (direction < 0 && Math.round(mImgRect.left) - direction >= mWidgetRect.left) false else !(direction > 0 && Math.round(mImgRect.right) - direction <= mWidgetRect.right)
    }

    fun canScrollVerticallySelf(direction: Float): Boolean {
        if (mImgRect.height() <= mWidgetRect.height()) return false
        return if (direction < 0 && Math.round(mImgRect.top) - direction >= mWidgetRect.top) false else !(direction > 0 && Math.round(mImgRect.bottom) - direction <= mWidgetRect.bottom)
    }
    ...
    private class InterpolatorProxy constructor() : Interpolator {
    ...
    fun animaFrom(info: Info) {
        if (isInit) {
            ...
            if (info.mWidgetRect.width() < info.mImgRect.width() || info.mWidgetRect.height() < info.mImgRect.height()) {
                ...
                clipX = (if (clipX > 1) 1 else clipX) as Float
                clipY = (if (clipY > 1) 1 else clipY) as Float
    ...
    fun animaTo(info: Info, completeCallBack: Runnable) {
        if (isInit) {
            ...
            mTranslate.withRotate(mDegrees.toInt(), info.mDegrees as Int, animaDuring * 2 / 3)
